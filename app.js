...
function load(){
  ...
  Promise.all([
    ...
    renderPub();updateCartBadge();buildHarvestCalendar();updateCustNav();loadProfileIntoAbout();
  }).catch(function(){
    loadFromLocal();
    renderPub();updateCartBadge();buildHarvestCalendar();updateCustNav();loadProfileIntoAbout();
  });
...
function renderProf(){
  ...
  // Load saved profile from localStorage/Firebase
  var saved=null;
  try{ var sp=localStorage.getItem('pp_maprofile'); if(sp) saved=JSON.parse(sp); }catch(e){}
  var prof = saved || {};
  var name    = prof.name    || CU.name    || 'P.A.O. Angel';
  var contact = prof.contact || CU.contact || '076 102 9880';
  var area    = prof.area    || CU.area    || 'Madampe';
  var bio     = prof.bio     || 'Passionate about connecting Sri Lankan farmers with modern households, I built PurePick to bring the freshest fruits and vegetables to your doorstep with zero compromise on quality.';
  var title   = prof.title   || 'Founder & Lead Seller — PurePick Digital Farmers Market';
  var branch1 = prof.branch1 || 'Madampe';
  var branch2 = prof.branch2 || 'Chilaw';
  var branch3 = prof.branch3 || 'Kuliyapitiya';
  var photoSrc= prof.photo   || '';

  // Update MA and CU with saved profile
  if(saved){ MA.name=name; MA.contact=contact; MA.area=area; CU.name=name; CU.contact=contact; CU.area=area; }

  var rows='';
  rows+=pr2('👤','Name',name);
  rows+=pr2('🔑','Username',CU.username);
  rows+=pr2('🎭','Role',isMa?'Main Admin (Full Access)':'Seller Admin');
  if(contact) rows+=pr2('📞','Contact',contact);
  if(area)    rows+=pr2('🏪','Area',area);
  if(!isMa){
  ...
  if(isMa) rows+=pr2('🏪','Branches',branch1+' • '+branch2+' • '+branch3);

  var avatarSrc = photoSrc || '';
  var avatarFallback = 'https://ui-avatars.com/api/?name='+encodeURIComponent(name)+'&background=1a5e2a&color=fff&size=200';

  document.getElementById('profbox').innerHTML=
    // ── VIEW CARD ──
    '<div class="pc2" style="max-width:100%;margin-bottom:1.5rem;">'+
      '<div class="pt">'+
        '<img id="prof-avatar" src="'+(avatarSrc||avatarFallback)+'" onerror="this.src=\''+avatarFallback+'\'" class="pav">'+
        '<div><div class="pnm">'+name+'</div><div class="prl">'+(isMa?'⭐ Main Administrator — PurePick':'🌿 Seller Admin')+'</div></div>'+
      '</div>'+
      '<div class="pfs">'+rows+'</div>'+
    '</div>'+
    // ── EDIT FORM (main admin only) ──
    (isMa?
    '<div style="background:#fff;border-radius:16px;padding:1.7rem;box-shadow:0 2px 13px rgba(0,0,0,.07);margin-bottom:1.2rem;">'+
      '<h3 style="font-family:\'Playfair Display\',serif;font-size:1.15rem;margin-bottom:1.3rem;color:var(--dk);">✏️ Edit About Section</h3>'+
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:.85rem;">'+
        '<div style="display:flex;flex-direction:column;gap:.35rem;"><label style="font-size:.79rem;font-weight:600;color:var(--tx);">Full Name</label>'+
        '<input type="text" id="ep-name" value="'+name+'" style="padding:.72rem .92rem;border:2px solid #e0e8d8;border-radius:8px;font-family:\'DM Sans\',sans-serif;font-size:.87rem;outline:none;"></div>'+
        '<div style="display:flex;flex-direction:column;gap:.35rem;"><label style="font-size:.79rem;font-weight:600;color:var(--tx);">Contact Number</label>'+
        '<input type="text" id="ep-contact" value="'+contact+'" style="padding:.72rem .92rem;border:2px solid #e0e8d8;border-radius:8px;font-family:\'DM Sans\',sans-serif;font-size:.87rem;outline:none;"></div>'+
        '<div style="display:flex;flex-direction:column;gap:.35rem;"><label style="font-size:.79rem;font-weight:600;color:var(--tx);">Home Area</label>'+
        '<input type="text" id="ep-area" value="'+area+'" style="padding:.72rem .92rem;border:2px solid #e0e8d8;border-radius:8px;font-family:\'DM Sans\',sans-serif;font-size:.87rem;outline:none;"></div>'+
        '<div style="display:flex;flex-direction:column;gap:.35rem;"><label style="font-size:.79rem;font-weight:600;color:var(--tx);">Title / Role Text</label>'+
        '<input type="text" id="ep-title" value="'+title+'" style="padding:.72rem .92rem;border:2px solid #e0e8d8;border-radius:8px;font-family:\'DM Sans\',sans-serif;font-size:.87rem;outline:none;"></div>'+
        '<div style="display:flex;flex-direction:column;gap:.35rem;grid-column:1/-1;"><label style="font-size:.79rem;font-weight:600;color:var(--tx);">Bio / Description</label>'+
        '<textarea id="ep-bio" style="padding:.72rem .92rem;border:2px solid #e0e8d8;border-radius:8px;font-family:\'DM Sans\',sans-serif;font-size:.87rem;outline:none;resize:vertical;min-height:80px;">'+bio+'</textarea></div>'+
        '<div style="display:flex;flex-direction:column;gap:.35rem;"><label style="font-size:.79rem;font-weight:600;color:var(--tx);">Branch 1</label>'+
        '<input type="text" id="ep-b1" value="'+branch1+'" style="padding:.72rem .92rem;border:2px solid #e0e8d8;border-radius:8px;font-family:\'DM Sans\',sans-serif;font-size:.87rem;outline:none;"></div>'+
        '<div style="display:flex;flex-direction:column;gap:.35rem;"><label style="font-size:.79rem;font-weight:600;color:var(--tx);">Branch 2</label>'+
        '<input type="text" id="ep-b2" value="'+branch2+'" style="padding:.72rem .92rem;border:2px solid #e0e8d8;border-radius:8px;font-family:\'DM Sans\',sans-serif;font-size:.87rem;outline:none;"></div>'+
        '<div style="display:flex;flex-direction:column;gap:.35rem;"><label style="font-size:.79rem;font-weight:600;color:var(--tx);">Branch 3</label>'+
        '<input type="text" id="ep-b3" value="'+branch3+'" style="padding:.72rem .92rem;border:2px solid #e0e8d8;border-radius:8px;font-family:\'DM Sans\',sans-serif;font-size:.87rem;outline:none;"></div>'+
        '<div style="display:flex;flex-direction:column;gap:.35rem;"><label style="font-size:.79rem;font-weight:600;color:var(--tx);">Profile Photo</label>'+
        '<div style="position:relative;width:100%;height:52px;border:2px solid #e0e8d8;border-radius:8px;background:#f8fbf6;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:.82rem;color:var(--mt);overflow:hidden;" onclick="document.getElementById(\'ep-photo-input\').click()">'+
          '📷 Upload Photo<input type="file" id="ep-photo-input" accept="image/*" style="position:absolute;inset:0;opacity:0;cursor:pointer;" onchange="previewProfilePhoto(this)">'+
        '</div></div>'+
      '</div>'+
      '<div style="display:flex;gap:.85rem;margin-top:1.3rem;">'+
        '<button onclick="saveProfile()" style="background:var(--g);color:#fff;padding:.75rem 1.8rem;border:none;border-radius:8px;font-family:\'DM Sans\',sans-serif;font-weight:700;cursor:pointer;font-size:.9rem;">💾 Save Profile</button>'+
        '<button onclick="resetProfile()" style="background:#f0f0f0;color:var(--tx);padding:.75rem 1.4rem;border:none;border-radius:8px;font-family:\'DM Sans\',sans-serif;font-weight:600;cursor:pointer;font-size:.9rem;">↩ Reset</button>'+
      '</div>'+
    '</div>'
    :'');
}
...
function saveProfile(){
  ...
  var contact = document.getElementById('ep-contact').value.trim();
  var area    = document.getElementById('ep-area').value.trim();
  var title   = document.getElementById('ep-title').value.trim();
  var bio     = document.getElementById('ep-bio').value.trim();
  var b1      = document.getElementById('ep-b1').value.trim();
  var b2      = document.getElementById('ep-b2').value.trim();
  var b3      = document.getElementById('ep-b3').value.trim();
  var photoEl = document.getElementById('prof-avatar');
  var photo   = photoEl ? photoEl.src : '';
  if(photo && photo.indexOf('ui-avatars')>=0) photo='';

  if(!name){showToast('⚠️ Name cannot be empty','er');return;}

  var prof={name:name,contact:contact,area:area,title:title,bio:bio,branch1:b1,branch2:b2,branch3:b3,photo:photo};

  // Save to localStorage
  localStorage.setItem('pp_maprofile',JSON.stringify(prof));

  // Save to Firebase
  fbSet('main_profile',prof);

  // Update live MA and CU
  MA.name=name; MA.contact=contact; MA.area=area;
  CU.name=name; CU.contact=contact; CU.area=area;

  // Update about section on the public page live
  updateAboutSection(prof);

  // Refresh dashboard nav name
  buildNav();
...
function resetProfile(){
  ...
  localStorage.removeItem('pp_maprofile');
  fbSet('main_profile',null);
  MA={username:'oshan',password:'oshan5555',name:'P.A.O. Angel',role:'main',area:'Madampe',contact:'076 102 9880'};
  CU.name=MA.name; CU.contact=MA.contact; CU.area=MA.area;
  showToast('↩ Profile reset to default','ok');
  renderProf();
}

function updateAboutSection(prof){
  // Update public about section live without page reload
  var nameEl=document.querySelector('.an'); if(nameEl) nameEl.textContent=prof.name||'P.A.O. Angel';
  var titleEl=document.querySelector('.at'); if(titleEl) titleEl.textContent=prof.title||'Founder & Lead Seller — PurePick Digital Farmers Market';
  var bioEl=document.querySelector('.abio'); if(bioEl) bioEl.textContent=prof.bio||'';
  var contactEl=document.querySelector('.adv[data-field="contact"]'); if(contactEl) contactEl.textContent=prof.contact||'';
  var areaEl=document.querySelector('.adv[data-field="area"]'); if(areaEl) areaEl.textContent=prof.area||'';
  // Update branch tags
  var btags=document.querySelectorAll('.bt');
  if(btags.length>=3){ btags[0].textContent='📍 '+prof.branch1; btags[1].textContent='📍 '+prof.branch2; btags[2].textContent='📍 '+prof.branch3; }
  // Update profile photo in about section
  var aphoto=document.querySelector('.aph');
  if(aphoto&&prof.photo) aphoto.src=prof.photo;
}

function loadProfileIntoAbout(){
  try{
    var sp=localStorage.getItem('pp_maprofile');
    if(sp){ var prof=JSON.parse(sp); updateAboutSection(prof); }
    else{
      // Try Firebase
      fbGet('main_profile').then(function(prof){
        if(prof) updateAboutSection(prof);
      });
    }
  }catch(e){}
}
